...
// Picking a deck's background, in Deck settings and in the settings of flashcards and Learn mode (the owner: "also allow
...
// The background tiles (BG_PICK_JS): in Deck settings, and in the settings of flashcards and Learn mode. Plain draws its
...
// `sw`: its lines' width (thinner when it's drawn big).
const CELL = (w, h, pointer = true, sw = 2) => `<svg width="${w}" height="${h}" viewBox="0 0 220 150" fill="none" stroke="{{t.text}}" stroke-width="${sw}"${pointer ? '' : ' style="display: block;"'}><ellipse cx="104" cy="80" rx="94" ry="62"/><circle cx="116" cy="74" r="24" fill="{{t.surf}}"/><circle cx="120" cy="70" r="7" fill="{{t.text}}"/><ellipse cx="54" cy="96" rx="16" ry="8"/><ellipse cx="74" cy="46" rx="12" ry="6"/><ellipse cx="158" cy="112" rx="14" ry="7"/>${pointer ? '<path d="M138 60 L186 22"/><circle cx="194" cy="16" r="12" fill="{{t.inv}}" stroke="none"/><text x="194" y="21" text-anchor="middle" font-size="13" font-weight="700" fill="{{t.invText}}" stroke="none" font-family="Geist, sans-serif">1</text>' : ''}</svg>`;
...
// The picture in the editor, with its boxes: drag on it to draw one; a picked box shows its corners and a ×. It's h
// tall; with no h (the big editor) it fills the room it's given, and the picture is as big as fits there.
const occPicture = h => `<div style="${h ? `height: ${h}px;` : 'flex: 1 1 0; min-height: 0; container-type: size;'} box-sizing: border-box; padding: 14px; border-radius: 20px; background: {{t.surf}}; display: flex; align-items: center; justify-content: center;">
...
// A big one (the big editor) fills the height it's given, in bigger type.
const field = (label, key, rows = 3, ph = '', big = false) => `<div style="${big ? 'flex: 1 1 0; min-height: 0; ' : ''}display: flex; flex-direction: column; gap: 8px;"><span style="font-size: 13px; font-weight: 600;">${label}</span><div class="sc-rich" contenteditable="true" role="textbox" aria-multiline="true" aria-label="${label}" spellcheck="true" data-rk="${key}" data-ph="${ph}" data-empty="{{rich.${key}.empty}}" key="{{rich.${key}.key}}" ref="{{rich.${key}.ref}}" style="position: relative; ${big ? 'flex-grow: 1; ' : ''}min-height: ${+(rows * 1.45).toFixed(2)}em; ${big ? '' : 'max-height: 14.5em; '}overflow-y: auto; border-radius: 20px; padding: ${big ? '18px 20px' : '14px 16px'}; background: {{t.surf}}; color: {{t.text}}; box-shadow: {{rich.${key}.ring}}; outline: 0; font-size: ${big ? 20 : 15}px; line-height: 1.45; white-space: pre-wrap; overflow-wrap: break-word; --ph: {{t.muted}}; transition: box-shadow .15s;">${RICH_EDIT(`rich.${key}.lines`)}</div></div>`;
...
const editorFieldsOf = (phone, big = false) => {
  ...
  return `
<sc-if value="{{isBasic}}" hint-placeholder-val="{{ true }}">${two(field('Front', 'front', 3, 'The question', big), field('Back', 'back', 2, 'The answer', big))}</sc-if>
<sc-if value="{{isCloze}}" hint-placeholder-val="{{ false }}">${two(field('Text', 'text', 3, 'Put [[double brackets]] around the words to hide', big), `
  ...
  ${field('Extra, shown after', 'note', 1, '', big)}`)}</sc-if>
<sc-if value="{{isImage}}" hint-placeholder-val="{{ false }}">${two(`<sc-if value="{{img.some}}" hint-placeholder-val="{{ true }}">${occPicture(picH)}</sc-if>
...
// The editor's Close, and its Delete, Cancel, and Save card (the panel's and the big editor's).
const EDITOR_CLOSE = `<a href="{{backHref}}" aria-label="Close" style="width: 36px; height: 36px; border-radius: 18px; background: {{t.surf}}; display: flex; align-items: center; justify-content: center;">${svg(I.close, 16, 2)}</a>`;
...
// A bigger card editor to try on the canvas (the owner: "card editor should be bigger i think especially for images");
// the app still opens the panel above. A wide sheet over the deck page: the kind of card, the formatting bar, and Close
// along the top, the card itself in two columns (editorFieldsOf), and its deck, tags, and Save along the bottom.
const webEditorBig = `<div style="position: relative; width: 1440px; height: 900px; overflow: hidden; font-family: ${FONT}; color: {{t.text}};">
  <dc-import name="WebDeck" dark="{{dark}}" dim="{{dim}}" deck-id="{{deckId}}" hint-size="1440px,900px"></dc-import>
  <div style="position: absolute; inset: 0; background: {{t.dim}};"></div>
  <aside role="dialog" aria-label="{{title}}" style="position: absolute; top: 24px; right: 24px; bottom: 24px; left: 24px; max-width: 1240px; margin: 0 auto; box-sizing: border-box; padding: 28px 32px; border-radius: 20px; background: {{t.bg}}; display: flex; flex-direction: column; gap: 24px; box-shadow: 0 0 0 1px {{t.line}}, 0 24px 64px rgba(0,0,0,.24);">
    <div style="display: flex; align-items: center; gap: 24px;"><div style="font-size: 22px; font-weight: 600; letter-spacing: -.02em;">{{title}}</div><div style="flex: 0 1 400px; min-width: 0;">${TYPE_SEG}</div><span style="flex-grow: 1;"></span><div style="display: flex; align-items: center; gap: 12px;">${WEB_FMT}${EDITOR_CLOSE}</div></div>
    ${editorFieldsOf(false, true)}
    <div style="display: flex; align-items: center; gap: 24px;"><div style="flex-grow: 1; display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">${chip(svg(I.decks, 12, 2) + '{{deckName}}', 'height: 32px; padding: 0 12px; font-size: 13px; font-weight: 600;')}${TAG_EDIT('cardTags', 'cardPick', false, true)}</div><div style="width: 420px;">${EDITOR_ACTIONS}</div></div>
    ${SLASH_MENU(420)}
  </aside>
</div>`;
const EDITOR_LOGIC = `
...
const donePilesLogic = phone => `renderVals() { ${T}${DB_JS}
  ...
  return { t, ...chrome, fill: 'linear-gradient(90deg, ' + ink[0] + ', ' + ink[1] + ')',
    ...
    moreHref: ss.moreHref, moreLabel: ss.moreLabel || 'Study 10 more', doneHref: db.mock ? '${phone ? 'PhoneDeck' : 'WebDeck'}.dc.html' : ss.doneHref }; }`;
const plainLogic = `renderVals() { ${T} return { t }; }`;
const doneLogic = (w, stroke, phone = false) => `renderVals() { ${T}${DB_JS}
  const ss = db.session();
  ${METER_JS(w, stroke)}
...
const phoneReview = `<div style="position: relative; isolation: isolate; width: 390px; height: 844px; box-sizing: border-box; padding: 60px 16px 34px; display: flex; flex-direction: column; gap: 16px; overflow: hidden; font-family: ${FONT}; background: {{t.bg}}; color: {{t.text}};">
  ${studyBgLayer}
  <div style="display: flex; align-items: center; gap: 12px;">
    ${roundBtn('close', 'End review', '{{endHref}}')}
...
const phoneDone = phone(`<div style="height: 100%; box-sizing: border-box; padding: 64px 20px 34px; display: flex; flex-direction: column; align-items: center; gap: 22px; text-align: center;">
  ...
  <div style="flex-grow: 1;"></div>
  <a href="{{doneHref}}" style="width: 100%; height: 58px; border-radius: 999px; background: {{t.inv}}; color: {{t.invText}}; display: flex; align-items: center; justify-content: center; font-size: 17px; font-weight: 600;">Done</a>
</div>`, '');
...
const learnSettings = phone => phone ? `<sc-if value="{{settingsOpen}}" hint-placeholder-val="{{ false }}">
    ...
    <div role="dialog" aria-label="Learn settings" style="position: absolute; left: 0; right: 0; bottom: 0; z-index: 5; box-sizing: border-box; padding: 10px 20px 34px; border-radius: 32px 32px 0 0; background: {{t.bg}}; color: {{t.text}}; display: flex; flex-direction: column; gap: 18px;">
      <div style="align-self: center; width: 40px; height: 5px; border-radius: 3px; background: {{t.surf2}};"></div>
      <div style="display: flex; align-items: center; justify-content: space-between;"><span style="font-size: 18px; font-weight: 600;">Learn settings</span><button type="button" onClick="{{toggleSettings}}" style="height: 36px; padding: 0 16px; border: 0; border-radius: 999px; background: {{t.inv}}; color: {{t.invText}}; font: inherit; font-size: 14px; font-weight: 600; cursor: pointer;">Done</button></div>
      ${bgChooser(true)}
    </div>
  </sc-if>` : `<sc-if value="{{settingsOpen}}" hint-placeholder-val="{{ false }}">
    <div role="dialog" aria-label="Learn settings" style="position: absolute; top: 68px; left: 32px; width: 380px; z-index: 5; box-sizing: border-box; padding: 20px; border-radius: 18px; background: {{t.bg}}; color: {{t.text}}; box-shadow: 0 0 0 1px {{t.line}}, 0 24px 64px rgba(0,0,0,.18); display: flex; flex-direction: column; gap: 18px;">
      <div style="display: flex; align-items: center; justify-content: space-between;"><span style="font-size: 16px; font-weight: 600;">Learn settings</span><button type="button" onClick="{{toggleSettings}}" aria-label="Close settings" style="width: 32px; height: 32px; border: 0; border-radius: 16px; background: {{t.surf}}; color: {{t.text}}; display: flex; align-items: center; justify-content: center; cursor: pointer;">${svg(I.close, 14, 2.2)}</button></div>
      ${bgChooser(false)}
    </div>
  </sc-if>`;
...
const files = {
  ...
  // The bigger editor, a mockup for the owner: nothing in the app opens it yet.
  'WebEditorBig': ['Web · Bigger card editor (mockup)', webEditorBig, { props: { ...DARK, cardType: { editor: 'enum', default: 'Basic', options: ['Basic', 'Blank', 'Image', 'Audio'] }, recording: { editor: 'boolean', default: false } }, logic: EDITOR_LOGIC, css: EDITOR_CSS, w: W, h: H }],
  'WebEditorBigImage': ['Web · Bigger card editor · image with boxes (mockup)', typeOf('WebEditorBig', W, H, 'Image'), { logic: darkLogic, css: EDITOR_CSS, w: W, h: H }],
  'WebEditorBigBlank': ['Web · Bigger card editor · fill in the blank (mockup)', typeOf('WebEditorBig', W, H, 'Blank'), { logic: darkLogic, css: EDITOR_CSS, w: W, h: H }],
