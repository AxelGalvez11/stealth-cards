...
for (const file of readdirSync(SRC).filter(f => f.endsWith('.dc.html')).sort()) {
  ...
  const phoneFill = ['PhoneSignIn', 'PhoneSignInCode', 'PhoneLibrary', 'PhoneDecksEmpty', 'PhoneQuizStart', 'PhoneQuizUpgrade', 'PhoneQuiz', 'PhoneQuizMatch', 'PhoneQuizType', 'PhoneQuizDone'].includes(name), fill = (w === 1440 && h === 900) || phoneFill;
