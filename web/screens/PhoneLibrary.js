// Made from design/canvas/project/PhoneLibrary.dc.html by design/to-web.mjs. Change the design, not this file.
export default {
  name: "PhoneLibrary", title: "iPhone · Library", w: 390, h: 844, fill: true,
  props: {"dark":false,"dim":false,"grain":0.7,"mode":"decks","folder":""},
  imports: [],
...
renderVals() {
  ...
  const liftTile = 'border-radius:20px!important;box-shadow:0 30px 60px -18px rgba(0,0,0,.5)!important;';
  ...
  const top = byUse.slice(0, 3), shownTags = tag === 'All' || top.includes(tag) ? top : [...top, tag];
  ...
  return {
    ...
    grabDeck: e => this.drag(e, { drops: ['folder:'], ink: t.text, bg: t.bg, lifted: view === 'cards' && !true ? liftTile : liftRow, bottom: 92, start: quiet,
      drop: (id, w) => (w.to ? act.moveDeck(id, w.to.slice(7) || null) : act.reorderDeck(id, w.before)) }),
    grabCard: e => this.drag(e, { drops: ['deck:'], reorder: false, keep: true, tray: all.length > 1, ink: t.text, bg: t.bg, lifted: liftRow, bottom: 92, start: quiet,
    ...
    hasMore: matched.length > s.shown, moreLabel: 'Show ' + Math.min(40, matched.length - s.shown) + ' more', showMore: () => this.setState({ shown: s.shown + 40 })
  };
}
}
return Component;
