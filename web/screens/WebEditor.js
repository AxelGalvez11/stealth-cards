...
export default {
  ...
  props: {"dark":false,"cardType":"Basic","recording":false,"slashDemo":false},
  imports: ["WebDeck"],
...
componentWillUnmount() { if (this.onSel) document.removeEventListener('selectionchange', this.onSel); const db = this.props.db; if (db && db.recording()) db.act.stopRecording(true); }
...
renderVals() {
  ...
  return {
    ...
    save: ev => {
      ...
      db.act.saveCard(saved ? saved.id : null, dk.id, { kind, front: f.front, back: f.back, text: f.text, note: f.note, tags, image: f.image || null, audio: f.audio || null, wave: f.audio ? f.wave || null : null, speak: f.speak || '', auto: f.auto !== false, clozeMode: f.clozeMode || 'each' }, backHref);
    }
  };
}
}
