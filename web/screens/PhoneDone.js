// Made from design/canvas/project/PhoneDone.dc.html by design/to-web.mjs. Change the design, not this file.
export default {
  name: "PhoneDone", title: "iPhone · Session done", w: 390, h: 844, fill: false,
  props: {"dark":false},
  imports: [],
  ...
  template: "<div style=\"position: relative; width: 390px; height: 844px; box-sizing: border-box; font-family: Geist, -apple-system, system-ui, sans-serif; background: {{t.bg}}; color: {{t.text}}; overflow: hidden;\">\n<div style=\"height: 100%; box-sizing: border-box; padding: 64px 20px 34px; display: flex; flex-direction: column; align-items: center; gap: 22px; text-align: center;\">\n  <div style=\"display: flex; flex-direction: column; align-items: center; gap: 10px;\"><div style=\"position: relative; width: 260px; height: 142px;\"><svg width=\"260\" height=\"142\" viewBox=\"0 0 260 142\" aria-hidden=\"true\"><defs><linearGradient id=\"sc-meter\" x1=\"0\" y1=\"0\" x2=\"1\" y2=\"0\"><stop offset=\"0\" stop-color=\"{{meter.a}}\"/><stop offset=\"1\" stop-color=\"{{meter.b}}\"/></linearGradient></defs><path d=\"M 16.0 130.0 A 114.0 114.0 0 0 1 244.0 130.0\" fill=\"none\" stroke=\"{{t.surf2}}\" stroke-width=\"20\" stroke-linecap=\"round\"/><path d=\"M 16.0 130.0 A 114.0 114.0 0 0 1 {{meter.x}} {{meter.y}}\" fill=\"none\" stroke=\"url(#sc-meter)\" stroke-width=\"20\" stroke-linecap=\"round\"/><circle cx=\"{{meter.x}}\" cy=\"{{meter.y}}\" r=\"15.0\" fill=\"{{t.bg}}\" stroke=\"{{meter.b}}\" stroke-width=\"4\"/></svg><div role=\"img\" aria-label=\"{{meter.pct}}% remembered\" style=\"position: absolute; left: 0; right: 0; bottom: 0; display: flex; flex-direction: column; align-items: center; gap: 2px;\"><span style=\"font-size: 52px; font-weight: 600; letter-spacing: -.04em; line-height: 1;\">{{meter.pct}}%</span><span style=\"font-size: 13px; color: {{t.muted}};\">remembered</span></div></div><span style=\"font-size: 14px; font-weight: 600; color: {{meter.ink}};\">{{goalLine}}</span></div>\n  <div style=\"display: flex; flex-direction: column; gap: 6px;\"><div style=\"font-size: 30px; font-weight: 700; letter-spacing: -.03em;\">Session complete</div><div style=\"font-size: 15px; color: {{t.muted}};\">{{summaryShort}}</div></div>\n  <div style=\"width: 100%; display: flex; flex-direction: column; gap: 10px; text-align: left;\"><div style=\"display: flex; height: 12px; border-radius: 6px; overflow: hidden; gap: 3px;\"><div style=\"width: {{split.w0}}; background: {{t.again}};\"></div><div style=\"width: {{split.w1}}; background: {{t.hard}};\"></div><div style=\"width: {{split.w2}}; background: {{t.good}};\"></div><div style=\"width: {{split.w3}}; background: {{t.easy}};\"></div></div>\n<div style=\"display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); font-size: 13px; color: {{t.muted}};\"><span>Forgot {{split.n0}}</span><span>Hard {{split.n1}}</span><span>Good {{split.n2}}</span><span>Easy {{split.n3}}</span></div></div>\n  <div style=\"width: 100%; display: flex; gap: 8px;\"><div style=\"flex-grow: 1; background: {{t.surf}}; border-radius: 22px; padding: 14px; text-align: left;\"><div style=\"font-size: 12px; color: {{t.muted}};\">Streak</div><div style=\"font-size: 20px; font-weight: 700;\">{{streakLabel}}</div></div><div style=\"flex-grow: 1; background: {{t.surf}}; border-radius: 22px; padding: 14px; text-align: left;\"><div style=\"font-size: 12px; color: {{t.muted}};\">Next</div><div style=\"font-size: 20px; font-weight: 700;\">{{nextShort}}</div></div></div>\n  <div style=\"flex-grow: 1;\"></div>\n  <a href=\"PhoneToday.dc.html\" style=\"width: 100%; height: 58px; border-radius: 999px; background: {{t.inv}}; color: {{t.invText}}; display: flex; align-items: center; justify-content: center; font-size: 17px; font-weight: 600;\">Done</a>\n</div>\n\n\n</div>",
  Logic: DCLogic => {
...
renderVals() { const t = this.theme(!!this.props.dark);const db = this.props.db || this.mock(); const chrome = db.chrome();
  const ss = db.session();
  const mr = 114, mcx = 130, mcy = 130;
