// Made from design/canvas/project/PhoneStats.dc.html by design/to-web.mjs. Change the design, not this file.
export default {
  name: "PhoneStats", title: "iPhone · Stats", w: 390, h: 844, fill: false,
  props: {"dark":false},
  imports: [],
...
renderVals() { const t = this.theme(!!this.props.dark);
  ...
  const heat = Array.from({ length: 17 * 7 }, (_, i) => { const v = (i * 37 + (i % 7) * 11) % 13; return cell(v < 3 ? 0 : v < 6 ? 1 : v < 9 ? 2 : v < 11 ? 3 : 4); });
  ...
  const fc = {"vals":[32,18,24,12,30,8,16],"labels":["Wed","Thu","Fri","Sat","Sun","Mon","Tue"],"tops":null,"names":["tomorrow","Thursday","Friday","Saturday","Sunday","Monday","Tuesday"]};
  const fcPeak = Math.max(0, ...fc.vals), fcMax = Math.max(1, fcPeak), fcTop = fc.vals.indexOf(fcPeak);
  const forecast = fc.vals.map((n, i) => ({ n: String(n), d: fc.labels[i], w: fc.tops ? fc.tops[i] : '', h: Math.max(6, Math.round(n / fcMax * 60)) + 'px',
