// Made from design/canvas/project/PhoneSignInCode.dc.html by design/to-web.mjs. Change the design, not this file.
export default {
  name: "PhoneSignInCode", title: "iPhone · Sign in · code from email", w: 390, h: 844, fill: true,
  props: {"dark":false},
  imports: [],
  ...
  template: "<div style=\"position: relative; width: 100%; min-height: 100vh; min-height: 100dvh; box-sizing: border-box; display: flex; flex-direction: column; font-family: Geist, -apple-system, system-ui, sans-serif; background: {{t.bg}}; color: {{t.text}}; overflow: hidden;\">\n<div style=\"flex: 1 0 auto; box-sizing: border-box; padding: 64px 20px 34px; display: flex; flex-direction: column; gap: 28px;\">\n  <a href=\"PhoneSignIn.dc.html\" aria-label=\"Use another email\" style=\"width: 44px; height: 44px; border-radius: 22px; background: {{t.surf}}; display: flex; align-items: center; justify-content: center;\"><svg width=\"18\" height=\"18\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M15 18l-6-6 6-6\"/></svg></a>\n  <div style=\"display: flex; flex-direction: column; gap: 8px;\"><h1 style=\"margin: 0; font-size: 32px; font-weight: 600; letter-spacing: -.03em;\">Check your email</h1><div style=\"font-size: 15px; line-height: 1.45; color: {{t.muted}};\">Enter the 6-digit code we sent to <span style=\"color: {{t.text}}; font-weight: 500;\">{{sentTo}}</span></div></div>\n  <label style=\"position: relative; display: flex; justify-content: space-between; gap: 6px; cursor: text;\"><sc-for list=\"{{boxes}}\" as=\"b\" hint-placeholder-count=\"6\"><span style=\"flex: 0 1 50px; min-width: 0; height: 58px; border-radius: 14px; background: {{t.surf}}; box-shadow: {{b.ring}}; display: flex; align-items: center; justify-content: center; font-size: 22px; font-weight: 600;\">{{b.digit}}</span></sc-for><input type=\"text\" inputmode=\"numeric\" autocomplete=\"one-time-code\" maxlength=\"6\" value=\"{{code}}\" onChange=\"{{setCode}}\" aria-label=\"6-digit code\" style=\"position: absolute; inset: 0; width: 100%; opacity: 0; border: 0; padding: 0; font-size: 16px;\"></label><sc-if value=\"{{hasError}}\" hint-placeholder-val=\"{{ false }}\"><div role=\"alert\" style=\"padding: 2px 4px 0; font-size: 14px; line-height: 1.4; color: {{t.again}};\">{{error}}</div></sc-if>\n  <div style=\"flex-grow: 1;\"></div>\n  <div style=\"display: flex; flex-direction: column; gap: 14px;\"><a href=\"PhoneToday.dc.html\" onClick=\"{{verify}}\" style=\"height: 52px; display: flex; align-items: center; justify-content: center; border-radius: 999px; background: {{t.inv}}; color: {{t.invText}}; font-size: 16px; font-weight: 600;\">{{verifyLabel}}</a><button type=\"button\" onClick=\"{{resend}}\" style=\"align-self: center; border: 0; padding: 0; background: transparent; color: {{t.muted}}; font: inherit; font-size: 15px; cursor: pointer;\">{{resendLabel}}</button></div>\n</div>\n</div>",
  Logic: DCLogic => {
...
renderVals() { const t = this.theme(!!this.props.dark);
  ...
  return { grain: String(this.props.grain ?? 0.7), t, sky: (this.props.dark ? { top: '#081733', mid: '#0D2148', low: '#0A1530', glow: 'rgba(120,150,255,.16)', cloud: 'rgba(150,170,230,.10)', wall: { top: '#081733', mid: '#0D2148', low: '#0A1530' } }
