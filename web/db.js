...
export async function createDb({ onChange, go }) {
  ...
  function face(c) {
    ...
    return { id: c.id, kind: c.kind, front: c.front || (c.kind === 'audio' ? 'What do you hear?' : ''), back: c.back, note: c.note, image: c.image, audio: c.audio, wave: c.wave || null, speak: c.speak, lang: c.lang || '',
