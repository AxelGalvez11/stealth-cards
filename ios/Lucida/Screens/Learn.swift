  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      VStack(alignment: .leading, spacing: 18) {
        ...
        Text("$39 a year, or $5.99 a month. Cancel anytime.").css(14).foregroundStyle(t.muted)
        FlexRow(spacing: 10) {
          Button { UIApplication.shared.open(URL(string: "https://lucida.cards/pricing")!) } label: {
            Text("See plans").css(15, .semibold).foregroundStyle(t.text).frame(maxWidth: .infinity).frame(height: 52).background(Capsule().fill(t.surf))
          }.buttonStyle(.press)
